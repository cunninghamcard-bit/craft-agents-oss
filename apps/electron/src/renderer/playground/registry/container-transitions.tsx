...
function IslandOptions({
  ...
  return (
    <motion.div className="flex w-[280px] shrink-0 flex-col gap-3 rounded-2xl border border-border/50 bg-background/90 p-3 border border-border backdrop-blur-sm">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="group inline-flex items-center gap-1.5 rounded-lg px-2 py-1 text-xs text-foreground/60 hover:bg-muted hover:text-foreground"
        >
          Reset
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className={cn('rounded-lg px-2.5 py-1.5 text-xs', view === 'compact' ? 'bg-foreground/10' : 'bg-muted hover:bg-foreground/10')}
        >
          Compact
        </button>
        <button
          ...
          className={cn('rounded-lg px-2.5 py-1.5 text-xs', view === 'confirm-follow-up' ? 'bg-foreground/10' : 'bg-muted hover:bg-foreground/10')}
        >
          Follow up
        </button>
        <button
          ...
          className={cn('rounded-lg px-2.5 py-1.5 text-xs col-span-2', view === 'confirm-ask-inline' ? 'bg-foreground/10' : 'bg-muted hover:bg-foreground/10')}
        >
          Ask inline
        </button>
      </div>
      ...
      <button
        ...
        className={cn(
          ...
          useMorph ? 'bg-foreground/10' : 'bg-muted hover:bg-foreground/10'
        )}
      >
        Morph from target (separate source offset): {useMorph ? 'On' : 'Off'}
      </button>

      <div className="rounded-xl border border-border/40 bg-muted-foreground/10 p-2.5 text-[11px] text-foreground/70 space-y-2">
        <div className="flex items-center justify-between">
          <span>Angle</span>
      ...
      <button
        ...
        className={cn(
          ...
          isIslandMounted
            ? 'bg-muted hover:bg-foreground/10 text-foreground/75'
            : 'bg-muted-foreground/10 text-foreground/35 cursor-not-allowed'
        )}
      >
      ...
      <div className="rounded-xl border border-border/40 bg-muted-foreground/10 p-2 text-[11px] text-foreground/65">
...
function ToolbarToConfirmTransitionDemo({ initialView = 'compact' }: ToolbarToConfirmTransitionDemoProps) {
  ...
  return (
    <div className="w-full max-w-[920px] p-6 space-y-4">
                  ...
                  <div className="w-[500px] p-3 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="h-7 w-7 inline-flex items-center justify-center rounded-[6px] text-foreground/70 hover:bg-muted hover:text-foreground"
                        aria-label="Back"
                      >
                    ...
                    <div className="rounded-[8px] border border-border/70 bg-muted-foreground/10 px-3 py-2">
                    ...
                    <div className="flex items-center gap-1 rounded-[8px] bg-muted-foreground/10 p-1 w-fit">
                      <button
                        type="button"
                        onClick={() => setAskScope('selection')}
                        className={cn(
                          'h-7 px-2.5 rounded-[6px] text-xs font-medium transition-colors',
                          askScope === 'selection' ? 'bg-background text-foreground border border-border' : 'text-foreground/65 hover:bg-muted'
                        )}
                      >
                        Selection only
                      </button>
                      <button
                        ...
                        className={cn(
                          'h-7 px-2.5 rounded-[6px] text-xs font-medium transition-colors',
                          askScope === 'full' ? 'bg-background text-foreground border border-border' : 'text-foreground/65 hover:bg-muted'
                        )}
                      >
                        Full response
                      </button>
                    ...
                    <div className="rounded-[8px] border border-border/70 px-3 py-2 bg-background border border-border">
                      <input
                        value={note}
      ...
      <div className="rounded-[10px] bg-muted-foreground/10 border border-border/40 px-3 py-2 text-xs text-foreground/70">
